...
function _termLoadProviders() {
    fetch('/api/config')
        ...
        .then(cfg => {
            ...
            if (!sel) return;
            const anthropicProviders = (cfg.providers || []).filter(p => p.type === 'anthropic');
            sel.innerHTML = '<option value="">' + (I18n.t('terminal.provider_none') || '– Provider wählen –') + '</option>';
            anthropicProviders.forEach(p => {
            ...
            if (anthropicProviders.length === 1) {
                sel.value = anthropicProviders[0].id || anthropicProviders[0].name;
            }
        })
        .catch(() => {});
}
