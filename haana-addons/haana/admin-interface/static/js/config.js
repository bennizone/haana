...
function addProviderWithType(type, authMethod) {
  ...
  _renderEmbeddingProviderDropdowns(cfg, cfg.embedding || {});
  Modal.close();
  renderProviders(cfg);
...
function _renderEmbeddingProviderDropdowns(c, em) {
  ...
  if (modelEl) modelEl.value = em.model || 'BAAI/bge-small-en-v1.5';
  updateEmbedDims();
  _updateEmbedLocalUI();
}
...
function _updateEmbedLocalUI() {
  ...
  if (isLocal && modelSelectEl) {
    ...
    const models = [
      ...
      { id: 'intfloat/multilingual-e5-large', dims: 1024, hint: t('config_memory.embed_model_multilingual') },
    ];
...
const _EMBED_DIMS = {
  'BAAI/bge-small-en-v1.5': 384, 'intfloat/multilingual-e5-large': 1024,
